...
To try the case, open AI Cost Lens, choose **Upload what you have**, select the Activity and Cost CSVs in **OpenAI bill review**, and build the review. The bill total and usage period should align. The cost export attributes **USD 487.20 to Support (86.4%)** and **USD 76.38 to QA (13.6%)**, so the review should prioritize investigating Support. Model-level billed cost and cost per usable result must remain unavailable: a project's cost is not a model's bill, and neither file records which results were usable. No savings claim follows from this case.

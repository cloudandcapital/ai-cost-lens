...
- Usage telemetry, request cost, and a user-entered billed total are separate exported layers. Telemetry supports operational diagnosis; a same-scope entered total supports an arithmetic comparison but does not verify invoice actuals, business value, or savings.
...
16. Compare a user-entered same-scope billed total and confirm that the export labels it unverified, with usage telemetry and request cost kept separate.

...
| Spend breakdown and run rate | Observability and FinOps tools segment spend and project run rate continuously | Local provider/model plus project/team/feature/customer/product/workload/workflow/session/environment breakdowns and average priced-row cost; 30-day run rate fails closed unless a complete 7+ day period, timestamps, prices, and one currency are confirmed | Implemented as a static snapshot |
| Cost allocation quality | Enterprise FinOps platforms expose allocation coverage and shared-cost treatment | Cost-weighted unallocated percentage for every supported allocation dimension; the user selects the decision dimension and warning threshold; period-level shared cost remains unallocated instead of being spread automatically | Implemented as a local decision check, not an allocation engine |
| Fully loaded AI operating cost | FinOps guidance extends beyond token or model invoices to infrastructure, data, tooling, and people | Optional compute, retrieval/data, network, tooling/observability, pipeline/orchestration, and human-review totals; blank stays unknown and fully loaded cost appears only when all six are supplied | Implemented with explicit same-period inputs |
| Usage telemetry versus billing evidence | Provider, gateway, and observability records arrive on different boundaries from finance bills | Exported evidence layers keep usage telemetry, provider/calculated request cost, and comparable billing evidence separate, with billed evidence confirming rather than silently replacing operational signals | Implemented |
...
| Optimization categories | Optimaizr and observability tools group findings into practical action areas | Findings carry billing integrity, reliability, caching, context reduction, output control, reasoning control, tooling, routing and downsizing, evidence, allocation, or variance categories | Implemented; every amount retains its own verification boundary |
...
| Finance decision posture | Finance stakeholders need a decision rather than a list of technical metrics | Route comparisons map to `FUND CHANGE`, `FIX EVIDENCE`, `STOP CHANGE`, or `INSUFFICIENT EVIDENCE`, while preserving the more specific operational next action such as test first or keep current route | Implemented |
...
- Explicit allocation fields through environment and session, with cost-weighted unallocated-share warnings tied to a user-selected decision dimension.
- A fail-closed operating-cost stack that refuses to display fully loaded cost while any declared category is unknown.
- Separate usage-telemetry, request-cost, and billing-evidence layers in the analysis record and interface.
- Plain optimization categories and a compact finance posture alongside the detailed action language.
...
1. **Automated release checks completed September 21, 2026.** The full Python suite passed 242 tests with five expected CCAC-validator skips; generated usage records passed their published JSON Schemas; Node interaction/static acceptance, production build, formatting, locked dependency audits, package install, and deterministic package smoke checks passed.

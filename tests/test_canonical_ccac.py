...
def test_real_local_1_0_reports_current_producer_and_adapter(tmp_path: Path):
    ...
    assert result["producer"]["version"] == "0.3.1"
    assert {source["adapter_version"] for source in result["inputs"]} == {"0.3.1"}
...
def test_1_1_emits_one_canonical_direct_ai_scope_with_exact_components():
    ...
    assert payload["producer"] == {"name": "ai-cost-lens", "version": "0.3.1"}

...
assert.equal(growthDemo.comparison.provider_cost_reported, false);
assert.equal(growthDemo.baseline.evidence.cost_basis, 'calculated');
assert.equal(((growthDemo.baseline.usage.processed_input_tokens - growthDemo.baseline.usage.cached_input_tokens) * 10 + growthDemo.baseline.usage.cached_input_tokens + growthDemo.baseline.usage.output_tokens * 50) / 1_000_000, growthDemo.baseline.costs.model_cost);
api.state.data = growthDemo;
api.renderAll();
assert.equal(element('decision-title').textContent, 'Test the lower-cost route');
assert.match(element('memo-rules').innerHTML, /Assumed approved for this invented comparison/);
assert.match(element('memo-evidence').innerHTML, /Invented model costs; no provider bill/);
assert.match(element('evidence-grid').innerHTML, /No provider bill or work log was verified/);
assert.match(api.lumenResponse('evidence'), /costs and outcomes are invented/);
const yieldCounts = [...element('outcome-yield-card').innerHTML.matchAll(/<div><strong>(\d+)<\/strong><span>(?:ready to use|need correction|need escalation)<\/span><\/div>/g)].map((match) => Number(match[1]));
assert.equal(yieldCounts.length, 6);
assert.equal(yieldCounts.slice(0, 3).reduce((a, b) => a + b), 100);
assert.equal(yieldCounts.slice(3).reduce((a, b) => a + b), 100);
...
assert.match(element('bill-opportunity-ledger').innerHTML, /USAGE ONLY.*Most requests went to gpt-economy/s);
assert.doesNotMatch(element('bill-opportunity-ledger').innerHTML, /Start with the busiest visible route/);
assert.match(element('bill-metric-ledger').innerHTML, /Blended cost per request/);
...
const fictionalBill = await api.buildOpenAIBillReview(read('tests/fixtures/fictional-asterdesk-openai-activity.csv'), read('tests/fixtures/fictional-asterdesk-openai-cost.csv'));
api.state.data = fictionalBill;
api.renderAll();
assert.equal(element('bill-project-cost').hidden, false);
assert.match(element('bill-project-cost-rows').innerHTML, /proj_aster_support.*\$487\.20.*86\.4%/);
assert.match(element('bill-opportunity-ledger').innerHTML, /Review billed cost for proj_aster_support/);
assert.match(element('bill-next-step').textContent, /proj_aster_support.*\$487\.20/);
assert.match(element('memo-next-step').textContent, /proj_aster_support.*Model billed cost remains unavailable/);
assert.match(element('bill-model-rows').innerHTML, /gpt-5\.6-luna[\s\S]*Unavailable/);
api.state.data = demo;
api.renderAll();

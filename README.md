...
- Cost-weighted allocation coverage across project, team, feature, customer, product, workload, workflow, session, and environment, with a user-set unallocated-cost warning
- A fail-closed operating-cost stack for compute, retrieval/data, network, tooling, pipeline, and human review; fully loaded cost appears only when every category is supplied
- Separate usage-telemetry, request-cost, and billing-evidence layers so a fast operational signal cannot masquerade as an invoice
- Plain optimization categories for caching, routing and downsizing, context, output, reasoning, tooling, reliability, evidence, allocation, and variance
- A finance posture of fund change, fix evidence, stop change, or insufficient evidence alongside the more specific next action
...
The OpenAI CSV importer is tested against contract-shaped fixtures and a CSV transformation of [OpenAI's published Usage and Costs API example](https://github.com/openai/openai-cookbook/blob/main/examples/completions_usage_api.ipynb). The transformation is **not** a native Usage Dashboard download; compatibility with an unmodified current customer CSV remains unverified. The published example exposed a partial first usage bucket even though its usage and cost dates matched, so the importer now checks exact UTC bucket boundaries before declaring the periods aligned. Do not present this test as a customer import or invoice reconciliation.
